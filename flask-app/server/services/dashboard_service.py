...
class DashboardService:
    ...
    def get_message_count_for_user(self, user_id: int):
        try:
            return MessageData.query.filter(
                (MessageData.sender_id == user_id) | (MessageData.recipient_id == user_id)
            ).count()
        except Exception as e:
            return 0
        
    def get_received_messages_for_user(self, user_id: int):
        try:
            return MessageData.query.filter(
                MessageData.recipient_id == user_id,
                MessageData.is_read == False  # Filter only unread messages
    ...
    def get_info_super_admin(self, user_id: int):
        ...
        try:
            ...
            message_count = self.get_message_count_for_user(user_id)
