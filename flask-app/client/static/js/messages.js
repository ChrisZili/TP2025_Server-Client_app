document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadAllMessages() {
    try {
      ...
      console.log("Fetched /messages/list response:", data);
  ...
  function renderMessagesCards(messages) {
    ...
    messages.forEach(msg => {
      ...
      card.addEventListener("click", async (event) => {
        ...
        try {
          if (!msg.is_read) {
            await fetch(`/messages/${msg.id}/mark_read`, {
              method: "PUT",
              credentials: "include"
            });
          }
          ...
          window.location.href = `/messages/${msg.id}`; // fallback
        }
      });
  ...
  document.getElementById("messages-list-cards").addEventListener("click", async (e) => {
    ...
    try {
      const resp = await fetch(`/messages/${msgId}/toggle_read`, {
        method: "PUT",
        credentials: "include",
        headers: { Accept: "application/json" }
      });
